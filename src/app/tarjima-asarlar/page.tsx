...
export const metadata: Metadata = {
  ...
  description:
    'Manbora tahririyati huquqiy asosi tekshirilgan holda o‘zbek tilida taqdim etgan tarjima asarlar.',
  alternates: { canonical: '/tarjima-asarlar' },
  openGraph: {
    type: 'website',
    title: 'Dunyo adabiyoti o‘zbek tilida',
    description:
      'Ingliz, rus va boshqa tillardan o‘zbekchaga tarjima qilingan sara kitoblar va hikoyalar.',
    url: '/tarjima-asarlar',
    siteName: 'Manbora',
    locale: 'uz_UZ',
  ...
  twitter: {
    ...
    title: 'Dunyo adabiyoti o‘zbek tilida',
    description: 'O‘zbekchaga tarjima qilingan sara kitoblar va hikoyalar.',
    images: ['/opengraph-image'],
  },
};
      ...
      <section className="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-950 to-emerald-800 p-6 sm:p-10 text-white">
        <div className="max-w-3xl space-y-3">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-bold">
            <Languages className="h-4 w-4" /> Tarjima asarlar
          </span>
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight">
            Dunyo adabiyoti o‘zbek tilida
          </h1>
          <p className="text-sm sm:text-base text-emerald-50/85 leading-relaxed">
            Original muallif nomi, tarjima qilingan til va tarjimon ma’lumoti ochiq ko‘rsatilgan
            saralangan asarlar.
          </p>
        </div>
      </section>
      ...
      <form className="flex flex-col sm:flex-row gap-3" action="/tarjima-asarlar">
        <label className="relative flex-1">
          ...
          <input
            ...
            placeholder="Asar yoki original muallifni qidiring"
            className="w-full rounded-2xl border border-stone-200 bg-white py-3 pl-11 pr-4 text-sm outline-none focus:border-emerald-600"
          />
        </label>
        <select
          name="language"
        ...
        <button className="rounded-2xl bg-emerald-800 px-6 py-3 text-sm font-bold text-white hover:bg-emerald-700">
          Qidirish
        </button>
      </form>
      ...
      {result.works.length ? (
        ...
        <div className="rounded-3xl border border-stone-200 bg-white py-16 text-center">
          ...
          <p className="mt-1 text-sm text-stone-500">
            Filtrni o‘zgartiring yoki keyinroq qayta kiring.
          </p>
        </div>
      )}
        ...
        <nav className="flex justify-center gap-2">
          {Array.from({ length: result.totalPages }, (_, i) => i + 1).map((n) => (
            <Link
              key={n}
              href={pageHref(n)}
              className={`rounded-xl px-4 py-2 text-sm font-bold ${n === page ? 'bg-emerald-800 text-white' : 'border border-stone-200 bg-white text-stone-700'}`}
            >
              {n}
            </Link>
          ))}
