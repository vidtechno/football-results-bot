...
export function SearchPageClient({
  ...
  const updateUrl = (newCategory: string, newRegion: string, newVerified: boolean) => {
    const params = new URLSearchParams();
    if (query) params.set('q', query);
  ...
  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setCategorySlug(val);
    updateUrl(val, regionSlug, verifiedOnly);
  };

  const handleRegionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const val = e.target.value;
    setRegionSlug(val);
    updateUrl(categorySlug, val, verifiedOnly);
  };

  const handleVerifiedToggle = () => {
    const nextVerified = !verifiedOnly;
    setVerifiedOnly(nextVerified);
    updateUrl(categorySlug, regionSlug, nextVerified);
  };
  ...
  const hasActiveFilters = Boolean(query || categorySlug || regionSlug || verifiedOnly);
