...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const now = Date.now();
  const cached = statsCache.get(params.id);
  ...
  try {
    const { data, error } = await db.rpc('get_work_public_stats', { p_work_id: params.id });
  ...
  if (!stats) {
    const [{ count: views }, { count: readers }, { count: bookmarks }, { count: completed }, { count: followers }] = await Promise.all([
      db.from('work_views').select('user_id', { count: 'exact', head: true }).eq('work_id', params.id),
      ...
      db.from('reading_progress').select('id', { count: 'exact', head: true }).eq('work_id', params.id).or('percentage.gte.100,is_completed.eq.true'),
  ...
  statsCache.set(params.id, { data: stats, timestamp: now });
