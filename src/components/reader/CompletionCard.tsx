...
export function CompletionCard({ workTitle, authorName, coverUrl }: { workTitle: string; authorName?: string | null; coverUrl?: string | null }) {
  ...
  const render = useCallback(async () => {
    ...
    ctx.fillStyle = '#78350f'; ctx.font = 'italic 900 48px Inter, Arial, system-ui, sans-serif'; ctx.fillText('Manbora', 72, 82);
    ...
    else { const fallback = ctx.createLinearGradient(875,158,1107,476); fallback.addColorStop(0,'#92400e'); fallback.addColorStop(1,'#166534'); ctx.fillStyle=fallback; ctx.fillRect(875,158,232,318); ctx.fillStyle='#fff7ed'; ctx.textAlign='center'; ctx.font='900 72px Inter, Arial, system-ui, sans-serif'; ctx.fillText('M',991,340); ctx.textAlign='left'; }
    ...
    ctx.fillStyle = '#1c1917'; ctx.font = `900 ${titleSize}px Inter, Arial, system-ui, sans-serif`; const titleLines = wrapText(ctx, workTitle, 700); titleLines.forEach((line, i) => ctx.fillText(line, 72, 275 + i * titleGap));
    const authorY = Math.min(455, 300 + titleLines.length * titleGap); ctx.fillStyle = '#57534e'; ctx.font = 'italic 600 27px Inter, Arial, system-ui, sans-serif'; ctx.fillText(authorName || 'Manbora muallifi', 74, authorY);
    ctx.fillStyle = '#92400e'; ctx.font = '600 24px Inter, Arial, system-ui, sans-serif'; ctx.fillText(`“${SLOGAN}”`, 74, 530);
  ...
  return <section className="overflow-hidden rounded-[2rem] border border-amber-300 bg-gradient-to-br from-amber-50 via-orange-50 to-emerald-50 p-5 text-center text-stone-900 shadow-xl shadow-amber-900/10 sm:p-8">
    ...
    <Award className="mx-auto mt-5 h-12 w-12 text-amber-600" /><h2 className="mt-2 font-sans text-2xl font-black sm:text-3xl">Asar tugadi — tabriklaymiz!</h2><p className="mx-auto mt-2 max-w-lg font-sans italic text-stone-600">“{SLOGAN}”</p>
