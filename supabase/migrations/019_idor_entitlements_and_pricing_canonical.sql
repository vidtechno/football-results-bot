...
-- Designed to be safely re-runnable on partially executed or fresh databases.
-- ============================================================================
...
BEGIN
  ...
  IF to_regclass('public.chapters') IS NOT NULL AND to_regclass('public.works') IS NOT NULL THEN
...
-- Ensure unique constraint exists idempotently
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'uq_user_work_chapter_entitlement'
  ) THEN
    ALTER TABLE public.entitlements
    ADD CONSTRAINT uq_user_work_chapter_entitlement
    UNIQUE (user_id, work_id, chapter_id, entitlement_type);
  END IF;
EXCEPTION
...
BEGIN
  IF to_regclass('public.entitlements') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Users can read own entitlements" ON public.entitlements';
    EXECUTE $pol$
...
DO $$
BEGIN
  IF to_regclass('public.entitlements') IS NOT NULL AND to_regclass('public.purchases') IS NOT NULL THEN
    EXECUTE $dyn$
      ...
      WHERE p.status IN ('active', 'completed', 'paid')
        AND NOT EXISTS (
          ...
          WHERE e.user_id = p.buyer_id
            AND e.work_id = p.work_id
            AND e.entitlement_type = CASE WHEN p.purchase_type = 'full_work' THEN 'full_work' ELSE 'chapter' END
            AND (
              (e.chapter_id IS NULL AND p.chapter_id IS NULL)
...
DO $$
BEGIN
  IF to_regclass('public.chapter_contents') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Read chapter content if free, purchased, author, or admin" ON public.chapter_contents';
    EXECUTE $pol$
...
BEGIN
  IF to_regclass('public.works') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Authors can update own works" ON public.works';
    EXECUTE $pol$
  ...
  IF to_regclass('public.chapters') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Authors can update own chapters" ON public.chapters';
    EXECUTE $pol$
  ...
  IF to_regclass('public.chapter_revisions') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Authors can insert own chapter revisions" ON public.chapter_revisions';
    EXECUTE $pol$
  ...
  IF to_regclass('public.work_revisions') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS "Authors can insert own work revisions" ON public.work_revisions';
    EXECUTE $pol$
...
DO $$
BEGIN
  IF to_regclass('public.in_site_notifications') IS NOT NULL
...
-- Backfills notifications for admin wallet top-ups using production public.wallet_transactions
DO $$
BEGIN
  IF to_regclass('public.in_site_notifications') IS NOT NULL
     AND to_regclass('public.wallet_transactions') IS NOT NULL
     AND to_regclass('public.wallet_accounts') IS NOT NULL THEN
    EXECUTE $dyn$
      INSERT INTO public.in_site_notifications (user_id, type, title, body, link_url, is_read, created_at)
      SELECT
        wa.user_id,
        ...
        'Administrator tomonidan balansingizga ' || TO_CHAR(wt.amount, 'FM999,999,999') || ' so‘m qo‘shildi.',
        '/kabinet?tab=transactions',
        false,
        wt.created_at
      FROM public.wallet_transactions wt
      JOIN public.wallet_accounts wa ON wa.id = wt.account_id
      WHERE wt.transaction_type IN ('topup', 'adjustment')
        AND wt.amount > 0
