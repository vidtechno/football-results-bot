...
export async function GET(req: NextRequest) {
  ...
  const [{ data: events }, { count: newToday }, { count: new7 }, { count: new30 }, { data: works }] = await Promise.all([
  ...
  return NextResponse.json({ visitorsToday: unique(rows.filter(x => x.created_at >= today.toISOString())), visitors7: unique(rows.filter(x => x.created_at >= since7)), visitors30: unique(rows), pageViewsToday: rows.filter(x => x.event_type === 'page_view' && x.created_at >= today.toISOString()).length, registrations: { today: newToday || 0, seven: new7 || 0, thirty: new30 || 0 }, devices: countBy('device_type'), sources: countBy('referrer_host'), topWorks: Object.entries(workCounts).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([id,count]) => ({ id, title: workNames.get(id) || 'Noma’lum asar', count })) }, { headers: { 'Cache-Control': 'private, no-store' } });
}
